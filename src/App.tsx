...
function App() {
  ...
  const [finalObjectArray, setFinalObjectArray] = useState<finalObjectType>([]);
  const [finalObject, setFinalObject] = useState<chessMoveType>();
  ...
  const yourFunction = (arg: string) => {
    ...
    setFinalObjectArray(finalObjectResult);
  ...
  const handleClick = (number: number) => {
    ...
    setCounter((prevCounter) => {
      const newCounter = prevCounter + number;
      if (newCounter >= 0 && newCounter < finalObjectArray.length) {
        setFinalObject(finalObjectArray[newCounter]);
        return newCounter;
      }
      return prevCounter; // Jeśli nowy licznik wychodzi poza zakres, nie aktualizujemy go
    });
  };
  ...
  return (
    <div>
      ...
      {finalObject ? <ChessBoard chessBoard={finalObject.chessBoard} /> : ""}
      <div className="flex">
        <div
          id="prev"
