...
function App() {
  ...
  useEffect(() => {
    setChessMove(chessMoveArray[counter]);
  }, [chessMoveArray, counter]);
  ...
  const handleClick = (number: number) => {
    setCounter((prevCounter) => {
      ...
      if (newCounter >= 0 && newCounter < chessMoveArray.length) {
        setChessMove(chessMoveArray[newCounter]);
        return newCounter;
      }
      return prevCounter;
    });
  ...
  return (
    <div>
      ...
      <div className="flex">
        ...
        {chessMoveArray ? (
          <MoveHistory chessMoveArray={chessMoveArray} setMove={setMove} />
        ) : (
          ""
        )}
      </div>
